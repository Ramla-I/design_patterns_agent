...
impl OwnershipDetector {
    ...
    pub async fn detect(
        ...
        let system_prompt = r#"You are an expert in Rust programming and design patterns. Your task is to analyze Rust code and identify ownership and lifetime invariants.

Ownership invariants ensure memory safety through:
- Lifetime annotations that express relationships between references
- Borrowing patterns (shared vs exclusive)
- Self-referential structures using Pin
- Interior mutability patterns (RefCell, Mutex, etc.)

For each invariant you find, provide:
1. A concise title
2. A description of the invariant
3. An explanation of how it ensures safety"#;

        let user_prompt = format!(
            r#"Analyze the following Rust code from module `{}` and identify any ownership or lifetime invariants:
...
If you find ownership invariants, respond in this format:
INVARIANT FOUND
Title: <title>
Description: <description>
Explanation: <explanation>

If no ownership invariants are found, respond with:
NO INVARIANTS FOUND"#,
            context.module_path,
            code_snippet,
            context.item.reason()
    ...
    fn parse_invariant_section(
        ...
        let invariant = Invariant {
            id: *next_id,
            invariant_type: InvariantType::Ownership,
            title,
            description,
            location: Location {
