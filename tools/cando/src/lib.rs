...
//!
//! ```ignore
//! // This glob-import is important for using the macro
//! use cando::*;
//!
//! candidate! {
//!     state: {
...
//!
//! Once defined, a binary can call `conduct()` from `main` in order to
//! use the runner as a CLI program.
//! See the documentation for `conduct` for information on how to use it.
...
/// The candidate macro has two forms.
///
...
macro_rules! candidate {
...

    {
        ...
        /// The directory of the compiled artifact to test.
        pub static ARTIFACT_DIR: OnceLock<PathBuf> = OnceLock::new();
        ...
        pub static LIBRARY: LazyLock<Library> = LazyLock::new(|| unsafe {
            let mut path = match ARTIFACT_DIR.get() {
                Some(p) => p.clone(),
                None => {
                    let mut default_path = CANDIDATE_DIR.clone();
                    default_path.push("build-ninja");
                    default_path
                }
            };
